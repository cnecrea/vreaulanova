"""
Modul de licențiere pentru integrarea Nova Power & Gas (vreaulanova).
...
- Câmpul `integration` identifică integrarea (fleet, eonmyline, vreaulanova, etc.)
...
class LicenseManager:
    """Gestionează licența pentru integrarea Nova Power & Gas.
    ...
    async def async_load(self) -> None:
        ...
        _LOGGER.debug("[Nova:License] Încep async_load()")
        try:
            stored = await self._store.async_load()
            self._data = dict(stored) if stored else {}
        except Exception:  # noqa: BLE001
            _LOGGER.warning(
                "[Nova:License] Storage corupt sau ilizibil "
                "— pornesc cu date goale (serverul va restaura starea)"
            )
            self._data = {}
        _LOGGER.debug(
            "[Nova:License] Date din storage: %d chei (%s)",
            len(self._data),
        ...
        _LOGGER.debug(
            "[Nova:License] Fingerprint generat: %s... (hw: %s...)",
        ...
        if self._status_token:
            ...
            _LOGGER.debug(
                "[Nova:License] Cache restaurat: status=%s, cache_valid=%s",
                cached_status,
                cache_valid,
            )
        else:
            _LOGGER.debug("[Nova:License] Niciun cache de status — prima rulare")

        # Verifică statusul la server (prima verificare la startup)
        _LOGGER.debug("[Nova:License] Verific statusul la server (startup)...")
        await self.async_check_status()

        self._loaded = True
        final_status = self.status
        _LOGGER.debug(
            "[Nova:License] async_load() finalizat — status=%s, is_valid=%s",
            final_status,
            self.is_valid,
        )
        ...
        if final_status == "licensed":
            ...
            _LOGGER.info(
                "[Nova:License] ✓ Licență ACTIVĂ (cheie: %s)", key
            )
        elif final_status == "trial":
            days = self.trial_days_remaining
            _LOGGER.info(
                "[Nova:License] ⏳ Perioadă de evaluare (trial): "
                "%d zile rămase", days
            )
        elif final_status == "expired":
            _LOGGER.warning(
                "[Nova:License] ✗ EXPIRAT — perioada de evaluare "
                "sau licența a expirat. Senzorii nu vor funcționa."
            )
        else:
            _LOGGER.warning(
                "[Nova:License] ✗ FĂRĂ LICENȚĂ (status=%s) — "
                "senzorii nu vor funcționa.", final_status
            )

    async def _async_save(self) -> None:
        """Salvează datele de licență."""
        _LOGGER.debug("[Nova:License] Salvez datele în storage")
    ...
    def _generate_fingerprint(self) -> str:
        ...
        - Salt: face fingerprint-ul specific integrării Nova Power & Gas
        """
        componente: list[str] = []
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        if self._is_status_cache_valid():
            _LOGGER.debug(
                "[Nova:License] Cache valid — folosesc token existent "
        ...
        _LOGGER.debug(
            "[Nova:License] Cache expirat sau inexistent — "
        ...
        try:
            ...
            async with session.post(
                ...
                headers={
                    ...
                    "User-Agent": "VreauLaNova-HA-Integration/1.0",
                },
            ) as resp:
                _LOGGER.debug(
                    "[Nova:License] Server /check răspuns: HTTP %d",
                    resp.status,
                )
                ...
                if resp.status == 200 and "status" in result:
                    ...
                    if not self._verify_token_signature(result):
                        _LOGGER.warning(
                            "[Nova:License] Semnătura token-ului de status "
                    ...
                    if server_key and self._data.get("license_key") != server_key:
                        ...
                        _LOGGER.debug(
                            "[Nova:License] license_key sincronizat "
                    ...
                    _LOGGER.debug(
                        "[Nova:License] Status actualizat de la server — %s "
                        "(valid_until: %s)",
                        server_status,
                    ...
                    if server_status == "expired":
                        _LOGGER.warning(
                            "[Nova:License] Server confirmă: EXPIRAT "
                            "(trial_days_remaining=0)"
                        )
                    elif server_status == "trial":
                        _LOGGER.info(
                            "[Nova:License] Server confirmă: TRIAL "
                            "(zile rămase: %s)",
                ...
                if result.get("error") == "invalid_hmac":
                    if self._data.get("client_secret") and not self._hmac_retry_done:
                        _LOGGER.warning(
                            "[Nova:License] HMAC invalid — client_secret "
                    ...
                    _LOGGER.error(
                        "[Nova:License] HMAC invalid (retry epuizat). "
                        "Serverul nu recunoaște acest dispozitiv."
                    )
                else:
                    _LOGGER.warning(
                        "[Nova:License] răspuns invalid de la /check — %s",
                        result,
                    )
                return self._status_token
            ...
            _LOGGER.warning(
                "[Nova:License] eroare de rețea la verificare status "
                "(încercare #%d) — %s",
            ...
            self._consecutive_failures += 1
            _LOGGER.warning(
                "[Nova:License] eroare neașteptată la verificare status "
                "(încercare #%d) — %s",
                self._consecutive_failures,
                err,
            )
    ...
    def is_licensed(self) -> bool:
        ...
        if not self._verify_token_signature(token):
            _LOGGER.warning("[Nova:License] semnătură token activare invalidă")
            return False

        # Verifică fingerprint-ul din token-ul de activare
        if token.get("fingerprint") != self._fingerprint:
            _LOGGER.warning("[Nova:License] fingerprint token nu se potrivește")
            return False

        # Verifică expirarea din token-ul de activare (licențe anuale)
        expires_at = token.get("expires_at")
        if expires_at and time.time() > expires_at:
            _LOGGER.info("[Nova:License] licența a expirat (token local)")
            return False
        ...
        if self._status_token and self._is_status_cache_valid():
            ...
            if server_status not in ("licensed", "trial"):
                _LOGGER.warning(
                    "[Nova:License] serverul raportează status '%s' — licență invalidă",
                    server_status,
                )
                return False
        ...
        if self._status_token and not self._is_status_cache_valid():
            if self._is_within_grace_period():
                if not self._cache_expiry_warned:
                    ...
                    _LOGGER.warning(
                        "[Nova:License] cache expirat — funcționare în perioadă "
                ...
                if not self._cache_expiry_warned:
                    _LOGGER.warning(
                        "[Nova:License] cache expirat + perioadă de grație depășită "
    ...
    def license_key_masked(self) -> str | None:
        """Returnează cheia de licență mascată (ex: NOVA-XXXX-****)."""
        key = self._data.get("license_key")
        ...
        try:
            ...
            async with session.post(
                f"{LICENSE_API_URL}/validate",
                    ...
                    _LOGGER.warning(
                        "[Nova:License] heartbeat respins — %s",
                        result.get("error", "necunoscut"),
                    )
                    return False

        except Exception:  # noqa: BLE001
            _LOGGER.debug("[Nova:License] heartbeat eșuat (rețea indisponibilă)")
            return False
        ...
        try:
            ...
            async with session.post(
                f"{LICENSE_API_URL}/activate",
                    ...
                    _LOGGER.debug(
                        "[Nova:License] /activate răspuns: HTTP %d",
                        resp.status,
                    )
                    ...
                    if result.get("success"):
                        ...
                        _LOGGER.info(
                            "[Nova:License] licență activată cu succes (%s)",
                            token.get("license_type", "necunoscut"),
                        )

                        # Auto-reload: reîncarcă toate entry-urile vreaulanova
                    ...
                    _LOGGER.warning(
                        "[Nova:License] activare eșuată — %s (răspuns: %s)",
                        error,
                        result,
                    )
            ...
            _LOGGER.error(
                "[Nova:License] eroare de rețea la activare — %s", err
            )
            return {"success": False, "error": "network_error"}
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[Nova:License] eroare neașteptată la activare — %s", err
            )
    ...
    async def async_deactivate(self) -> dict[str, Any]:
        ...
        try:
            ...
            async with session.post(
                f"{LICENSE_API_URL}/deactivate",
                    ...
                    if resp.status == 200 and result.get("success"):
                        ...
                        _LOGGER.info(
                            "[Nova:License] licență dezactivată cu succes"
                        )
            ...
            _LOGGER.error("[Nova:License] eroare la dezactivare — %s", err)
            return {"success": False, "error": "network_error"}
                ...
                f"{LICENSE_API_URL}/notify",
                    ...
                    if resp.status == 200:
                        ...
                        if not result.get("success"):
                            _LOGGER.warning(
                                "[Nova:License] Server a refuzat '%s': %s",
                                action, result.get("error"),
                            )
                    else:
                        _LOGGER.warning(
                            "[Nova:License] Notify HTTP %d pentru '%s'",
                            resp.status, action,
                        )
        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[Nova:License] Nu s-a putut raporta '%s': %s",
                action, err,
            )
    ...
    async def _async_reload_entries(self) -> None:
        """Reîncarcă toate entry-urile vreaulanova după activare/dezactivare.
        ...
        _LOGGER.info(
            "[Nova:License] Reîncarc %d entry-uri după schimbarea licenței",
            len(entries),
        )
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            _LOGGER.warning(
                "[Nova:License] nicio cheie publică nu a validat semnătura"
            )
            return False

        except Exception as err:  # noqa: BLE001
            _LOGGER.debug(
                "[Nova:License] verificare semnătură eșuată — %s", err
            )
            return False
